...
def run_all_tests(df):
  ...
  # Iterate over each combination of two questions
  for i, question1 in enumerate(questions):
    try: 
        for j, question2 in enumerate(questions[i+1:]):
        # Check the data type of the questions
        question1_type = df[question1].dtype
        question2_type = df[question2].dtype
        ...
        results = results.append({'question1': question1, 'question2': question2, 'test': test, 'statistic': statistic, 'p_value': p_value, 'count': df[question2].count(), 'mean': df[question2].mean(), 'sum': df[question2].sum()}, ignore_index=True)
